...
网页端使用保存在 `DATA_DIR/.uploads` 的可续传分片会话，因此不需要调大反向代理常见的 1 MiB 请求体限制。ZIP/CBZ 和图片文件夹均按 768 KiB 分片、单部漫画最多四路并发上传。未完成和失败会话从最后一次活动起保留 24 小时；已排队和处理中的会话不会按期限删除，进程重启后会自动恢复。每部漫画源文件最多 10 GiB、10000 页，ZIP 解压后的图片总量最多 20 GiB。分片齐全后由服务端异步解压和入库，页面轮询处理状态。

导入成功后，服务端会先删除上传分片和临时解压数据，再向网页端公开结果。网页端收到结果后会删除服务端会话；成功项目仍显示在当前页面，直到用户清除或离开。无人确认的成功结果最多兜底保留 24 小时。过期记录会立即从接口隐藏，并在应用启动时或每小时维护时从磁盘删除。“清除已完成”也会永久删除对应服务端会话，因此刷新后不会再次出现。

现有 `POST /api/admin/import/upload` multipart 接口及其字段和响应继续保留，供旧客户端使用。该接口仍是同步请求，受现有 Nginx 请求体和超时限制；网页端新上传使用分片会话接口。失败会话会保留源分片，直到重试、取消或过期。容器部署时请确保 `DATA_DIR` 使用持久化卷。
